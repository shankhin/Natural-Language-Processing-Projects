...
import json
import itertools
from operator import concat, ge
import random
...
def build_current_surrounding_pairs(indices: "list[int]", window_size: int = 2):
    ...
    for index in range(len(indices)): 
        ...
        surroundings.append([indices[i] 
                                 for i in range(start, end) 
                                 if 0 <= i < len(indices)
                                 and i != index])
    news = list()
    currents = indices[window_size:len(indices)-window_size]
    news.append([x for x in surroundings if len(x) == 2*window_size])
    news = [e for sl in news for e in sl]
    return  news, currents


def expand_surrounding_words(ix_surroundings: "list[list[int]]", ix_current: "list[int]"):
    # TODO: your work here
    s = list(itertools.chain(*ix_surroundings))
    c = list()
    for x in ix_current:
        for i in range(len(ix_surroundings[0])):
            c.append(x)
    return s, c
...
def cbow_preprocessing(indices_list: "list[list[int]]", window_size: int = 2):
    ...
    for x in indices_list:
        surroundings, current = build_current_surrounding_pairs(x,window_size)
        c.extend(current)
        s.extend(surroundings)
    return s, c
...
def skipgram_preprocessing(indices_list: "list[list[int]]", window_size: int = 2):
    ...
    for x in indices_list:
        surroundings, current = build_current_surrounding_pairs(x,window_size)
        surroundings_exp, current_exp = expand_surrounding_words(surroundings, current)
        c.extend(current_exp)
        s.extend(surroundings_exp)
    return s, c
...
class SharedNNLM:
    def __init__(self, num_words: int, embed_dim: int):
        ...
        self.projection = nn.Linear(embed_dim, num_words, False)
...
def compute_topk_similar(
    ...
    # TODO: your work here 
    cos = F.cosine_similarity(word_emb,w2v_emb_weight,1)
    ranking = torch.argsort(cos, descending=True)
    return ranking.tolist()[1:k+1]
...
def retrieve_similar_words(
    ...
    model.eval()
    torch.no_grad()
    word_idx = index_map.get(word)
    w2v_emb_weight = model.emb.weight
    word_emb = model.emb.weight[word_idx]
    indices_list = compute_topk_similar(word_emb,w2v_emb_weight,k)
    word_list = [index_to_word[x] for x in indices_list]
    return word_list
...
def word_analogy(
    ...
    word_a_idx = index_map.get(word_a)
    word_b_idx = index_map.get(word_b)
    word_c_idx = index_map.get(word_c)

    w2v_emb_weight = model.emb.weight

    word_a_emb = model.emb.weight[word_a_idx]
    word_b_emb = model.emb.weight[word_b_idx]
    word_c_emb = model.emb.weight[word_c_idx]

    word_res_emb = word_a_emb - word_b_emb + word_c_emb
    indices_list = compute_topk_similar(word_res_emb,w2v_emb_weight,k)
    word_list = [index_to_word[x] for x in indices_list]
    return word_list
...
def compute_gender_subspace(
    ...
    # TODO: your work here
    gender_emb = list() #list[list(str,str)]
    for x in gender_attribute_words:
        gender_emb.append([word_to_embedding[x[0]],word_to_embedding[x[1]]])
    for l in gender_emb:
        mean = (l[0]+l[1])/2
        l[0] = l[0] - mean
        l[1] = l[1] - mean
    gender_emb = np.array(gender_emb)
    gender_emb = gender_emb.reshape(-1, gender_emb.shape[-1])
...
def compute_profession_embeddings(
    ...
    # TODO: your work here
    profession_split = list()

    for profession in professions:
        profession_split.append(profession.split())

    profession_emb = list()
    for profession in profession_split:
        sum = 0
        for word in profession:
            emb = word_to_embedding[word]
            sum+= emb
        mean = sum/len(profession)
        profession_emb.append(mean)
    dicts = {}
    for i,x in enumerate(professions):
        dicts[x] = profession_emb[i]
    return dicts
...
def compute_extreme_words(
    ...
    #word emb is list of 60 arrays of shape (50,)
    #so (60,50)
    word_scalar_list = list()
    for i in range(gender_subspace.shape[0]):
        for j in range(len(word_emb)):
            s, v = project(word_emb[j],gender_subspace[i])
            word_scalar_list.append((words[j],s))
    
    word_scalar_list.sort(key=lambda x:x[1])

    l = list()
    for t in word_scalar_list:
        l.append(t[0])
    
    lrev = list(reversed(l))

    if max_ == True:
        extreme_words = lrev[:k]
    else:
        extreme_words = l[:k]
...
def debias_word_embedding(
    ...
    for i in range(gender_subspace.shape[0]):
            s, projection = project(word_emb,gender_subspace[i])
...
if __name__ == "__main__":
    ...
    # Prefilled code showing you how to use the helper functions
    word_to_embedding = load_glove_embeddings("data/glove/glove.6B/glove.6B.300d.txt")
    ...
    # === Section 2.5 ===
    # direct_bias_professions = compute_direct_bias(professions,profession_to_embedding,gender_subspace,0.25)
    # print(f"DirectBias Professions (before debiasing): {direct_bias_professions:.2f}")
    # === Section 2.6 ===
    ...
    # === Section 3.1 ===
    # debiased_word_to_embedding = hard_debias(word_to_embedding,gender_attribute_words,1)
    # debiased_profession_to_embedding = compute_profession_embeddings(debiased_word_to_embedding, professions)

    # # === Section 3.2 ===
    # direct_bias_professions_debiased = compute_direct_bias(professions,debiased_profession_to_embedding,gender_subspace,0.25)
    ...
    # print(f"p-value (after debias): {p_value_debiased:.2f}")
    ...
    words = list()
    words.extend(min_words)
    words.extend(max_words)

    word_emb = list()
    for word in words:
        word_emb.append(profession_to_embedding[word])

    tsne = TSNE(n_components = 2, random_state = 123, init = 'pca', perplexity=10)
    z = tsne.fit_transform(word_emb)
